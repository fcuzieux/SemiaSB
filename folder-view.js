        ...
        item.addEventListener('click', async () => {
            ...
            chrome.downloads.search({ query: [filename] }, (results) => {
                if (results && results.length > 0) {
                    ...
                    // Si aucun match dans le dossier spécifique, on retombe sur le comportement par défaut (tous les résultats)
                    // Cela permet d'ouvrir le fichier même si l'utilisateur a changé de dossier de sauvegarde entre temps
                    if (matches.length === 0) {
                        matches = results;
                    }
