...
// Chargement asynchrone des prompts depuis les fichiers Markdown
(async function loadAIPrompts() {
  try {
    const fetchPrompt = async (filename) => {
      const response = await fetch(`ai/ai-skills/${filename}`);
      if (!response.ok) throw new Error(`Erreur HTTP: ${response.status}`);
      return await response.text();
    };

    const results = await Promise.allSettled([
      fetchPrompt('minutes.md'),
      fetchPrompt('summary.md'),
      fetchPrompt('pimp.md')
    ]);

    if (results[0].status === 'fulfilled') AI_PROMPT_TEMPLATES.minutes = results[0].value;
    if (results[1].status === 'fulfilled') AI_PROMPT_TEMPLATES.summary = results[1].value;
    if (results[2].status === 'fulfilled') AI_PROMPT_TEMPLATES.pimp = results[2].value;
    
    console.log("✅ Prompts IA chargés depuis ai-skills/");
  } catch (error) {
    console.error("❌ Erreur lors du chargement des prompts IA:", error);
  }
})();
